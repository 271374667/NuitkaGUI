...
class PluginModel:
    ...
    def get_enable_plugins(self) -> List[str]:
        return self._command_manager.command_plugin.enable_plugins
    ...
    def get_all_packages_by_py_file(self, py_file: Path) -> List[str]:
        ...
        return self._command_manager.command_plugin.filter_plugins(list(set(dependence_list)))

    def set_plugin_status(self, plugin_name: str, status: bool) -> None:
        self._command_manager.command_plugin.set_plugin_enable(plugin_name, status)

    def disable_all_plugin(self) -> None:
        self._command_manager.command_plugin.disable_all_plugin()

    def fetch_plugin_from_cmd(self) -> List[Tuple[str, str]]:
        return self._command_manager.command_plugin.fetch_plugin_from_cmd()
